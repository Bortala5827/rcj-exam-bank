...
> RCJ 招考生态三大仓库之一，定位**综合公职考试**（国考 / 省考 / 事业编 / 教师招聘 / 三支一扶 / 社区工作者 / 军队文职）。
...
```
实例 = config.js（品牌/文案/模块开关） + data-written.json / data-interview.json（题库） + 生成的 index.html
```
...
每个考试类型目录放：`config.js`、`data-written.json`、`data-interview.json`、构建出的 `index.html`。
...
2. 准备题库 `data-written.json` / `data-interview.json`（用 `rcj-exam-builder` 的 `ingest.py` 从 CSV/PDF/Word 转）。
...
原 `civil-service-interview` 的面试内容**并入本仓库**作为板块；旧仓库已归档并 README 重定向。参见 `_migration/civil-service-interview.md`。
