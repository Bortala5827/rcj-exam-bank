...
async function joinOrWait(db, city, myPeerId) {
  ...
  const now = Date.now();
  try {
    await db.prepare("DELETE FROM signal_wait WHERE created_at < ?").bind(now - 24e4).run();
  } catch (e) {
  }
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    const list = (results || []).filter(function(x) {
      return x.peer_id !== myPeerId && (x.created_at || 0) > now - 24e4;
    });
    if (list.length > 0) {
      const partner = list[0];
      const del = await db.prepare("DELETE FROM signal_wait WHERE ticket=? AND peer_id=?").bind(partner.ticket, partner.peer_id).run();
      ...
      const exp = now + MATCH_TTL * 1e3;
      try {
        await db.prepare("INSERT OR REPLACE INTO signal_match (ticket,room,is_host,role,peer_id,expires) VALUES (?,?,1,?,?,?)").bind(partner.ticket, room, partnerRole, myPeerId, exp).run();
        await db.prepare("INSERT OR REPLACE INTO signal_match (ticket,room,is_host,role,peer_id,expires) VALUES (?,?,0,?,?,?)").bind(myTicket, room, myRole, partner.peer_id, exp).run();
        return { ok: true, matched: true, ticket: myTicket, room, isHost: false, role: myRole, peerId: partner.peer_id };
      } catch (e) {
        await new Promise(function(r) {
          setTimeout(r, 50);
        });
      ...
      try {
        await db.prepare("INSERT OR REPLACE INTO signal_wait (ticket,city,peer_id,created_at) VALUES (?,?,?,?)").bind(myTicket, city, myPeerId, now).run();
...
async function cancelWait(db, t) {
  try {
    await db.prepare("DELETE FROM signal_wait WHERE ticket=?").bind(t).run();
  } catch (e) {
  }
  return { ok: true };
}
...
async function onRequestPost(context) {
  ...
  if (action === "cancel") {
    const city2 = cityOf(url.searchParams.get("city"));
