...
```
  ...
  独立产品（自有后端 / 品牌 / 边界清晰）→ 走子域名，被各 hub 链接出去：
    ...
    xf.955827.xyz     ← xf-firefighter-exam  消防刷题站（笔试+面试）
    facetalk.955827.xyz ← facetalk          FaceTalk 面试搭子（P2P 匹配）
```

**设计铁律**：轻量工具进 hub 子路径；独立产品（有自己后端 / 独立品牌 / 产品边界清晰）走子域名。
Training（体测·体能）轻、无后端 → 作 `/train` 子路径；辅警/消防/FaceTalk 是完整产品 → 独立子域。二者不互相吞并。
...
| `xf-firefighter-exam` | `xf.955827.xyz` | 消防刷题站（笔试+面试） | 独立 CF Pages 项目，自定义域 |
...
> 辅警/消防题库**不是**本仓库的一部分，而是被 Exam Hub 在导航/卡片里**链接出去**的独立产品。请勿将二者合并进同一仓库——那会重踩「子路径同步坑」（需 cp 副本 + 双推）且动到已稳定的自定义域。
