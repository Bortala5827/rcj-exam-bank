...
> 本地源文件放工作区根目录 **`_media/V*.mp4`**（与 `_repos`、`projects` 同级，**独立于 GitHub 仓库**），不推送 Pages。
...
1. **本地加工**：用本机 ffmpeg 把原始视频压制成小体积（如 `-c:v libx264 -crf 29 -preset slow -pix_fmt yuv420p -movflags +faststart -c:a aac -b:a 64k`），输出到 `_media/V{序号}_{主题}.mp4`。**不要直接用手机上传原始大文件**。
2. **上传 Telegram**：把 `_media/V*.mp4` 发给 `@rcj_tg_store_bot`（或发到频道 `1004447493424`），拿到 `file_id`。
